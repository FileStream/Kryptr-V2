...
namespace KryptrGUI
{
    ...
    public partial class MainWindow : Window
    {
        ...
        {
            MessageBox.Show("Allows KV2 to utilize more than 1 will make the process of encryption/decryption faster but at the cost of using all of your computer's processing power and slowing other programs down when encrypting/decrypting very large groups of files.", "What is it to utilize full processing power?", MessageBoxButton.OK, MessageBoxImage.Information);
        }

        private void ShowKeepOriginalHelp(object sender, RoutedEventArgs e)
        {
            MessageBox.Show("This checkbox simply tells KV2 whether you'd like to keep the original files after they are encrypted or decrypted. This is left unchecked by default so as to not keep both an unecrypted copy and an encrypted copy of each which may be unsafe and take up storage space.", "What does \"keep original files\" do?", MessageBoxButton.OK, MessageBoxImage.Information);
        }

        private void ShowCompLevelHelp(object sender, RoutedEventArgs e)
        {
            MessageBox.Show("This dropdown allows you to tell KV2 how much you'd like it to compress files. Higher levels may make files significantly smaller, but will make the encryption and decryption process longer. Lower levels will encrypt and decrypt faster, but will leave larger files.\n\nNOTE: If double-layered encryption is enabled, this setting applies to both the automatic compression and the second layer of compression.", "What does the compression level dropdown do?", MessageBoxButton.OK, MessageBoxImage.Information);
        }

        private void ShowScrambleHelp(object sender, RoutedEventArgs e)
        {
            MessageBox.Show("The scramble filenames option will set KV2 to randomize the filenames and remove the extensions of encrypted files so as to make it harder for others to guess the contents. This is off by default so that you can still easily tell what the file originally was.", "What does scramble filenames do?", MessageBoxButton.OK, MessageBoxImage.Information);
        }
